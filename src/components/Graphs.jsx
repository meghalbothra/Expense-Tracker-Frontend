...
const Graphs = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const incomeResponse = await axios.get(`https://expense-tracker-backend-rav8.onrender.com/api/v1/auth/income?userId=${userId}`, {
        headers: {
          Authorization: `Bearer ${token}` // Include token in headers
        }
      });
      const expenseResponse = await axios.get(`https://expense-tracker-backend-rav8.onrender.com/api/v1/auth/expenses?userId=${userId}`, {
        headers: {
          Authorization: `Bearer ${token}` // Include token in headers
        }
      });
